...
sectionCtrl.create = async (req, res) => {
  try {
      ...
      console.error('Error creating section:', error); // Log the error to the console
...
sectionCtrl.list = async (req, res) => {
    try {
        const sections = await Section.find().populate('tasks'); // Populate tasks in each section
